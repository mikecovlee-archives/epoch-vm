...
namespace cs {
	namespace shared {
		...
		public:
			...
			{
				if (empty())
					epoch::throw_ex<epoch::runtime_error>("Stack is empty.");
				return *(m_current - 1);
			}
			...
			{
				if (offset >= size())
					epoch::throw_ex<epoch::runtime_error>("Stack out of range.");
			...
			{
				if (full())
					epoch::throw_ex<epoch::runtime_error>("Stack overflow.");
			...
			{
				if (empty())
					throw epoch::runtime_error("Stack is empty.");
				(m_current - 1)->~T();
				--m_current;
			}
